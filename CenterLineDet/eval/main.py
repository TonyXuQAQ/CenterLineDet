...
parser.add_argument('-matching_threshold', action='store', dest='matching_threshold', type=float,
                    help='topo marble-hole matching distance ', required =False, default=0.00002)

parser.add_argument('-interval', action='store', dest='topo_interval', type=float,
                    help='topo marble-hole interval ', required =False, default=0.00001)
...
create_directory('../%s/results/topo'%args.savedir,delete=True)
image_list = os.listdir(os.path.join('../',args.savedir,'pred_graph'))
image_list.sort()
image_list = [x for x in image_list if '.p' in x]
for tile_idx in tqdm(image_list):
    tile_idx = tile_idx[:-2]
    graph_prop = '../%s/pred_graph/%s.p'%(args.savedir,tile_idx)
    graph_gt = '../../dataset/label_gt_graph/%s.p'%tile_idx
    ...
    lmap = topo.TOPOGeneratePairs(graph_prop, graph_gt, losm, threshold = 0.00003, region=region)

    # propagation distance 
    r = 0.00100 # around 100 meters
