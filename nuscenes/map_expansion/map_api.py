...
class NuScenesMap:
    ...
    def get_centerlines_graph(self,resolution_meters):
        return self.explorer.get_centerlines_graph(resolution_meters)
    ...
    def get_centerlines_graph(self, resolution_meters: float) -> List[np.array]:
        """
        """
        pose_lists = []
        for lane in self.lane + self.lane_connector:
    ...
    def get_graph(self, resolution_meters: float) -> List[np.array]:
        """
        Discretize the centerlines of lanes and lane connectors.
        :param resolution_meters: How finely to discretize the lane. Smaller values ensure curved
            lanes are properly represented.
        ...
        for lane in self.lane + self.lane_connector:
            my_lane = self.arcline_path_3.get(lane['token'], [])
            discretized = np.array(discretize_lane(my_lane, resolution_meters))
            data_list = [[round(x[0],5),round(x[1],5),round(x[2],5)] for x in discretized.tolist()]
            output_list.append({'start_node':data_list[0],'end_node':data_list[-1],
                                'vertices':data_list})
...
class NuScenesMapExplorer:
    ...
    def get_centerlines_graph(self,resolution_meters):
        return self.map_api.get_centerlines_graph(resolution_meters)
