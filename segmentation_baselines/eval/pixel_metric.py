...
from segtograph.segtograph import post_process_skeleton
...
create_directory(f'../{args.root_dir}/results')
create_directory(f'../{args.root_dir}/vector_pred',True)
create_directory(f'../{args.root_dir}/vector_gt',True)
create_directory(f'../{args.root_dir}/pred_graph',True)
create_directory(f'../{args.root_dir}/pred_csv',True)
...
with tqdm(total=len(image_list),unit='img') as pbar:
    for ii, image_name in enumerate(image_list):
        pred_mask = (np.array(Image.open(os.path.join('../',args.root_dir,'pred_mask',image_name)))/255>0.15)
        ...
        pred_mask = post_process_skeleton(pred_mask,'../'+args.root_dir,image_name[:-4],save_graph=('multi' in args.name))
        Image.fromarray(pred_mask).save(os.path.join('../',args.root_dir,'vector_pred',image_name))
