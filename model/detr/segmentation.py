...
class DETRsegm(nn.Module):
    def __init__(self, detr, freeze_detr=False):
        ...
        # # self.segmentation_head = FPN(output_size=detr.args.ROI_half_length*2)
        self.mask_head = MaskHeadSmallConv(hidden_dim + nheads, [1024, 512, 256], hidden_dim, output_size=self.detr.args.ROI_half_length*2)
    ...
    def forward(self, samples: NestedTensor):
        ...
        out["pred_ahead_masks"] = outputs_seg_masks
...
class FPN(nn.Module):
    ...
    def forward(self,fpns):
        ...
        feature = self._upsample(self.conv_fuse(s2 + s3 + s4 + s5), self.output_size, self.output_size)
