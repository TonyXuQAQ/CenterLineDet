...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, args):
        ...
        self.bce_loss = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([5]).cuda())
        self.args = args
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_focal = self.label_focal_loss(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        losses = {'loss_ce': loss_ce, 'loss_focal': loss_focal}
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_line = F.l1_loss(src_boxes, target_boxes)

        losses = {}
        losses['loss_bbox'] = loss_line
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        target_masks_raw, valid = nested_tensor_from_tensor_list(masks).decompose()
        # upsample predictions to the target size
        src_masks = interpolate(src_masks, size=target_masks_raw.shape[-2:],
                                mode="bilinear", align_corners=False)
        src_masks = src_masks.flatten(1)
        target_masks = target_masks_raw.flatten(1)

        
        losses = {
            "loss_mask": self.bce_loss(src_masks, target_masks)
        }
        return losses
    
    def loss_ahead_mask(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)
        src_masks = outputs["pred_ahead_masks"]
        src_masks = src_masks[src_idx]
        masks = [t["ahead_mask"] for t in targets]
        # TODO use valid to mask invalid areas due to padding in loss
        ...
        losses = {
            "loss_ahead_seg": self.bce_loss(src_masks, target_masks) 
        }
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'ahead_mask': self.loss_ahead_mask
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)
    
    def empty_get_loss(self, loss, outputs, targets, **kwargs):
        loss_map = {
            'labels': self.loss_labels
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, None, None, **kwargs)
    ...
    def forward(self, outputs, targets):
        ...
        if len(targets):
            ...
            losses = {}
            for loss in self.losses:
                losses.update(self.empty_get_loss(loss, outputs, targets, None, num_boxes))
        return losses
...
def build(args):
    ...
    losses = ['labels', 'boxes', 'ahead_mask']
